async function initSimulationPage() {
  ...
  function renderMeters() {
    ...
    meters.innerHTML = Object.keys(labels)
      .map((key) => {
        const value = App.clamp(Math.round(scores[key]), 0, 100);
        const tone = key === "peace" ? "meter__fill--alternative" : "";
        return `
          <div class="meter">
            <div class="meter__label"><span>${labels[key]}</span><span>${value}%</span></div>
            <div class="meter__track"><div class="meter__fill ${tone}" style="width:${value}%"></div></div>
          </div>
        `;
      })
      .join("");

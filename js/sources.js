async function initSourcesPage() {
  ...
  if (about && data.about) {
    const paragraphs = data.about.paragraphs
      .map((p) => `<p class="body-text" style="margin-bottom:1rem;">${p}</p>`)
      .join("");

    const authorBlock = data.about.author
      ? `
        <div class="about-block">
          <h3 class="about-block__title">${data.about.author.label}</h3>
          <p class="body-text about-block__name">${data.about.author.name}</p>
          <p class="body-text">
            <a href="mailto:${data.about.author.email}">${data.about.author.email}</a>
          </p>
          <p class="body-text">
            <a href="tel:${data.about.author.phone}">${data.about.author.phone}</a>
          </p>
        </div>
      `
      : "";
    ...
    function listBlock(section) {
      ...
      return `
        <div class="about-block">
          <h3 class="about-block__title">${section.title}</h3>
          <ul class="about-list">
    ...
    about.innerHTML = `
      ...
      ${paragraphs}
      ${authorBlock}
      ${listBlock(data.about.stack)}
      ${listBlock(data.about.architecture)}
    `;
  }
